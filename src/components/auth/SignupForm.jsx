...
import { User, Mail, Lock, AlertCircle, CheckCircle, Loader2, MailCheck } from 'lucide-react';
...
export default function SignupForm({ onSuccess, onSwitchToLogin }) {
  ...
  const validatePassword = (pwd) => {
    ...
    return { isValid: hasUppercase && hasLowercase && hasNumber && hasSpecial && hasLength,
  ...
  const handleMembershipComplete = () => {
    ...
    // After membership modal, show the verify-email prompt
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        setSignedUpName(formData.name);
        setSignedUpEmail(formData.email);
        // Show membership modal first, verify prompt comes after
  ...
  const handleResendVerification = async () => {
    ...
    try {
      await authService.post('/auth/resend-verification', { email: signedUpEmail });
  ...
  // ── Email verification prompt shown after signup + membership modal ──────────
  if (showVerifyPrompt) {
    return (
      <div className="text-center space-y-6 py-4">
        ...
        <div>
          ...
          <p className="text-gray-600">
            ...
            <strong className="text-blue-900">{signedUpEmail}</strong>.
        ...
        {resendSuccess ? (
          ...
          <button
            ...
            className="text-sm text-blue-900 hover:underline font-semibold disabled:opacity-50"
          >
        ...
        <button
          ...
          className="w-full px-4 py-3 rounded-lg bg-[#8B1A1A] text-white font-bold hover:bg-[#7a1616] transition-colors"
        >
          Back to Login
        </button>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <p className="text-xs text-gray-500 leading-relaxed pt-1">
            ...
            <Link href="/terms" target="_blank" rel="noopener noreferrer"
              className="text-blue-900 font-semibold underline underline-offset-2 hover:text-blue-700">
              Terms &amp; Conditions
            </Link>{' '}
            and{' '}
