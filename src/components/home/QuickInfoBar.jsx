  ...
  return (
    <section className="py-0 bg-white overflow-hidden">
      <div className="max-w-[1600px] mx-auto md:px-6">
        <div className="flex flex-col lg:flex-row bg-slate-900 md:rounded-[4rem] overflow-hidden min-h-[800px] shadow-2xl">
          ...
          <div className="lg:w-7/12 bg-white p-8 md:p-20 flex flex-col justify-center">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-1">
              {quickLinks.map((item, index) => {
                const Icon = item.icon;
                return (
                  <Link key={index} href={item.href} className="group py-8 border-b border-slate-50 hover:border-[#8B1A1A] transition-all duration-500">
                    <div className="flex items-center justify-between gap-4">
                      <div className="space-y-1 flex-1">
                        <div className="flex items-center gap-2">
                          <span className="text-[10px] font-black text-[#8B1A1A] opacity-40">0{index + 1}</span>
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] group-hover:text-[#8B1A1A] transition-colors">
                            {item.label}
                          </p>
                        </div>
                        <h3 className="text-xl font-black text-slate-900 tracking-tight group-hover:translate-x-2 transition-transform duration-500">
                          {item.value}
                        </h3>
                      </div>
                      
                      <div className="size-12 rounded-2xl bg-slate-50 flex items-center justify-center text-slate-300 group-hover:bg-[#8B1A1A] group-hover:text-white transition-all duration-500 rotate-45 group-hover:rotate-0">
                        <Icon size={20} />
                      </div>
                    </div>
                  </Link>
            ...
            <div className="mt-16 flex items-center justify-between pt-8 border-t border-slate-100">
              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                Need Help? <span className="text-slate-900 ml-2">Contact Ministry Office</span>
              </p>
