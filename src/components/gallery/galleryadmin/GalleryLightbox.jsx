...
const GalleryLightbox = ({
  ...
  // All hooks FIRST - before any return/condition
  useEffect(() => {
    if (isOpen && photo && allPhotos.length > 0) {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e) => {
      ...
      if (e.key === 'ArrowLeft') setCurrentIndex(prev => (prev === 0 ? allPhotos.length - 1 : prev - 1));
      if (e.key === 'ArrowRight') setCurrentIndex(prev => (prev === allPhotos.length - 1 ? 0 : prev + 1));
    };

    window.addEventListener('keydown', handleKeyDown);
    return () => window.removeEventListener('keydown', handleKeyDown);
  }, [isOpen, allPhotos.length, onClose]);
    ...
    <div className="fixed inset-0 bg-black z-50 flex items-center justify-center p-4 overflow-hidden">
        ...
        <div className="max-w-4xl max-h-[90vh] w-full h-full flex items-center justify-center bg-gray-100 rounded-lg">
          <img
            ...
            onError={(e) => {
              ...
              e.target.parentElement.innerHTML = '<div class="text-center text-gray-500 p-4">Image failed to load</div>';
            }}
          />
        </div>
      </div>
      ...
      {hasMultiple && (
        <>
          <button
            ...
            className="absolute left-4 top-1/2 -translate-y-1/2 p-2 hover:bg-white/20 rounded-lg transition z-10 hover:scale-110"
          >
          ...
          <button
            ...
            className="absolute right-4 top-1/2 -translate-y-1/2 p-2 hover:bg-white/20 rounded-lg transition z-10 hover:scale-110"
          >
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-6 z-10">
        <div className="max-w-4xl mx-auto">
          <div className="mb-4">
            <h2 className="text-2xl font-bold text-white mb-2">{currentPhoto.title || 'Untitled'}</h2>
